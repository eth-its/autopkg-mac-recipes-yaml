...
:<<DOC
MacTeX installs BibDesk.app into the /Applications/TeX folder,
so we use an AutoPkg recipe that repackages BibDesk.app so that
...
tex_location='/Applications/TeX/BibDesk.app'
standalone_location='/Applications/BibDesk.app'

# Check that the installation worked
if [[ -d "$tex_location" ]]; then
    tex_version=$(get_version "$tex_location")
    echo "BibDesk.app installation found at $tex_location. Version: $tex_version"
else
    echo "BibDesk.app installation not found at $tex_location. Version: $tex_version"
    tex_version=0
fi
...
if [[ -d "$standalone_location" ]]; then
    ...
    echo "BibDesk.app installation found at $standalone_location. Version: $standalone_version"
else
    echo "BibDesk.app installation not found at $standalone_location. Version: $standalone_version"
