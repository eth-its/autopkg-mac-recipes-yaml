...
supplied version string or higher - modified to optionally accept a prefix to inject into the smartgroups
to handle version numbers like "PrusaSlicer PrusaSlicer-2.9.5" via SMARTGROUP_VERSION_PREFIX 

originally by G Pugh, Modified by cglattfelder
...
class VersionRegexGeneratorPrefix(Processor):
    ...
    input_variables = {
        ...
        "SMARTGROUP_VERSION_PREFIX": {
            "required": False,
            "description": "A prefix that should not be evaluated,but is still part of the version",
            "default": None,
        },
    }
...
if __name__ == "__main__":
    PROCESSOR = VersionRegexGeneratorPrefix()
